...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initDocumentsPage(user, profile) {
    ...
    /* -----------------------------------------------------
       STAFF VIEW — see and review ALL documents, no upload
    ----------------------------------------------------- */
    if (profile.role === 'staff') {
      ...
      var staffDocsQuery = query(collection(db, 'documents'), orderBy('uploadedAt', 'desc'));
      var staffList = document.getElementById('documentsList');

      onSnapshot(staffDocsQuery, function (snapshot) {
        if (snapshot.empty) {
          staffList.innerHTML = '<p class="documents-empty-state">No documents have been submitted yet.</p>';
          return;
        }

        staffList.innerHTML = snapshot.docs.map(function (docSnap) {
          var d = docSnap.data();
          ...
          return (
            '<article class="document-item">' +
              '<div class="document-item-main">' +
                ...
                '<div><h4>' + escapeHtml(d.docType || 'Document') + ' — ' + escapeHtml(d.studentName || 'Unknown') + '</h4>' +
                '<p>Uploaded ' + dateStr + ' • <a href="' + d.fileURL + '" target="_blank" rel="noopener">View File</a></p></div>' +
              '</div>' + actions +
            '</article>'
          );
        }).join('');

        staffList.querySelectorAll('[data-action]').forEach(function (btn) {
      ...
      return; // staff view ends here — skip the student/parent upload logic below
    }
