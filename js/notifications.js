...
export function wireNotificationBell(uid) {
  ...
  watchMyNotifications(uid, function (notifications, err) {
    if (err) {
      list.innerHTML = '<p class="list-empty-state" style="color:var(--color-red); font-family:monospace; font-size:11px;">' + escapeHtmlLocal(err.message) + '</p>';
      dot.hidden = true;
      return;
    }
