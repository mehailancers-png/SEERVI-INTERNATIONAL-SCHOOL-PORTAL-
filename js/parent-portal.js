  ...
  function initPortal(user, profile) {
    ...
    if (isPreview) {
      ...
      banner.innerHTML = '<span class="info-note-icon">🔍</span><span>' +
        '<strong>Staff Preview Mode</strong> — you are viewing the Parent Portal as staff, under your own account. ' +
        'Linking a child and photo upload are disabled here to avoid affecting any real account.</span>';
    ...
    if (isPreview && linkForm) {
      linkForm.querySelectorAll('input, button').forEach(function (el) { el.disabled = true; });
    }
    ...
    function watchLinkedAttendance(studentUid) {
      onSnapshot(doc(db, 'attendance', studentUid), function (docSnap) {
        var tbody = document.getElementById('parentAttendanceTableBody');
        var statGrid = document.getElementById('parentAttendanceStatGrid');

        if (!docSnap.exists()) {
          tbody.innerHTML = '<tr><td colspan="4">No attendance published for your child yet.</td></tr>';
          statGrid.hidden = true;
          return;
        }

        var months = (docSnap.data().months || []).filter(function (m) { return (m.present + m.absent) > 0; });
        if (months.length === 0) {
          tbody.innerHTML = '<tr><td colspan="4">No attendance published for your child yet.</td></tr>';
          statGrid.hidden = true;
          return;
        ...
        document.getElementById('parentStatAttendancePercent').textContent = overallPct + '%';
        document.getElementById('parentStatDaysPresent').textContent = totalPresent;
        document.getElementById('parentStatDaysTotal').textContent = totalDays;
          ...
          return (
            '<article class="document-item">' +
              '<div class="document-item-main">' +
                ...
                '<p>Uploaded ' + dateStr + reviewLine + ' • <a href="' + d.fileURL + '" target="_blank" rel="noopener">View File</a></p>' +
                reasonLine +
                '</div>' +
              '</div>' +
